...
>
	...
	<xsl:param name="structure_stylesheet" select="document(/html/head/s:structure_stylesheet)/s:structure_stylesheet"/>
	...
	<xsl:template match="*[@id|@class]" mode="structure_stylesheet">		
	...
	<xsl:template name="gather_matching_rules" mode="structure_stylesheet">
		<xsl:param name="class_string"/>
	...
	<xsl:template name="find_most_specific_rule" mode="structure_stylesheet">		
		<xsl:param name="matching_rules"/>		
		<xsl:param name="current_node"/>		
	...
	<xsl:template match="s:element" mode="structure_stylesheet">
		...
		<xsl:element name="{name($current_node)}">
	...
	<!--s:element (change tag name)-->
	<xsl:template match="s:element[@change_tag]" mode="structure_stylesheet">
		<xsl:param name="current_node"/>
		
		<!--replace s:element with current node (give current node a new tag name)-->		
		<xsl:element name="{@change_tag}">						
			<!--update or copy attributes-->
			<xsl:call-template name="update_attributes">		
				<xsl:with-param name="current_node" select="$current_node"/>
			</xsl:call-template>
			
			<!--copy contents of current node-->
			<xsl:apply-templates select="node()" mode="structure_stylesheet"/>						
		</xsl:element>		
	</xsl:template>	
	...
	<xsl:template match="s:contents" mode="structure_stylesheet">
		<xsl:param name="current_node"/>
		<!--replace s:contents with current node contents (cycle begins again on contents of current node)-->
