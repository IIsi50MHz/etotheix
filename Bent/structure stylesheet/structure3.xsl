...
>  
	...
	<xsl:template match="s:structure_stylesheet" mode="structure_stylesheet">		
		...
		<!--see if current node has a class or id-->
		
		<xsl:if test="normalize-space($class_string) or $id_string">
			...
			<xsl:variable name="matching_rules" select="$matching_id_rules|$matching_class_rules"/>			
			
			<xsl:choose>
				<!--keep looking for rules if there are any matches-->
				<xsl:when test="$matching_rules">
					<xsl:call-template name="gather_matching_rules">
						<xsl:with-param name="class_string" select="substring-after($class_string, $class_name)"/>	
						<xsl:with-param name="matching_rules" select="$matching_rules"/>
						<xsl:with-param name="current_node" select="$current_node"/>
					</xsl:call-template>
				</xsl:when>
				<!--just copy node if there are no rule matches-->	
				<xsl:otherwise>
	...
	<xsl:template name="gather_matching_rules">
		...
		<xsl:if test="normalize-space($class_string)">
			<xsl:call-template name="gather_matching_rules">
				<xsl:with-param name="class_string" select="substring-after($class_string, $class_name)"/>
	...
	<xsl:template match="*" mode="restructure_current_node_contents">			
		...
		<xsl:copy>
			...
			<xsl:apply-templates select="$element/node()" mode="structure_stylesheet">
				<xsl:with-param name="current_node" select="."/>
			</xsl:apply-templates>
		</xsl:copy>
	</xsl:template>
